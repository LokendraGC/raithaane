  ...
  var rtsJs = {
    ...
    cartNumberIncDec: function () {
      ...
      $(".coupon-click").on("click", function () {
        $(this)
          .parents(".coupon-input-area-1")
          .find(".coupon-input-area")
          .toggleClass("show");
      });
    ...
    modalpopupCard: function () {
      ...
      $(".section-activation").on("click", function () {
        $(".section-activation"),
          $(this).parents(".item-parent").addClass("deactive");
      });
    },
...
(function ($) {
  wtn = {
    ...
    remove_cart_mini: () => {
      ...
      jQuery(".remove_from_cart_button").on("click", function (e) {
        jQuery.ajax({
          ...
          url: ajaxURL,
          data: {
            action: "custom_remove",
          },
          success: function (response) {
            if (response.success) {
              if (response.data.cart_count == 0) {
                $(".cart-number span").remove();
              } else {
                $(".cart-number span").text(response.data.cart_count);
              }
            }
          },
        });
    ...
    saerch_filter: () => {
      ...
      jQuery(".filter_product").on("click", function (e) {
        e.preventDefault();
        var order = jQuery(".price_by").val();
        var weight = jQuery(".weight_by").val();
        ...
        jQuery.post(ajaxURL, data, function (response) {
          ...
          jQuery("#product_count").html(response.data.product_count);
          jQuery("#page_no").val("2");

          if (response.data.product_count > 8) {
            $("#load_more").show();
          } else {
            $("#load_more").hide();
          }
    ...
    reset_filter: () => {
      ...
      jQuery(".reset_filter").on("click", function (e) {
        ...
        // reset price
        jQuery(".price_by .option").removeClass("selected");
        jQuery('.price_by .option[data-value="Sort By"]').addClass("selected");
        jQuery(".price_by .current").text("Sort By");

        // reset weight
        jQuery(".weight_by .option").removeClass("selected");
        jQuery('.weight_by .option[data-value="Filter By weight"]').addClass(
          "selected"
        );
        jQuery(".weight_by .current").text("Filter By weight");

        jQuery(".price_by").val("");
        jQuery(".weight_by").val("");
    ...
    load_more_products: () => {
      var ajaxURL = RAITHAANEobj.ajaxurl;
      var page_count = parseInt(jQuery(".page_count").val());
...
$(document).ready(function () {
  ...
  // Remove nice-select if applied
  if (billingCountrySelect.hasClass("nice-select")) {
    billingCountrySelect.niceSelect("destroy"); // Remove nice-select plugin
  }

  // Ensure the original select element is displayed
  billingCountrySelect.css("display", "block");

  // Hide the div with the classes 'nice-select', 'country_to_state', 'country_select'
  $("div.nice-select.country_to_state.country_select").css("display", "none");
});
